...
console.log(dogs);
//2
const marinaDog = dogs.find(function(dog) {
  return dog.owners.includes("Marina")
});
console.log(marinaDog);

console.log(marinaDog.curFood > marinaDog.recommendedFood ? "Собака переедет" : "Собака недоедает");
//3
...
//4
console.log(`Собаки ${ownersEatTooMuch.join(" и ")} едят слишком много! Собаки ${ownersEatTooLittle.join(" и ")} едят слишком мало`)
...
//7
const normalDogs = dogs.filter(dog => dog.curFood > dog.recommendedFood * 0.9 && dog.curFood < dog.recommendedFood * 1.1);
